...
public class JsonParser {
	...
	protected final static String JSON_KEY_BEST = "isBest";
	protected final static String JSON_KEY_FLAG = "flag";
	protected final static String JSON_KEY_PROCESSTYPE = "processType";
    ...
    private static boolean getBooleanValue(JSONObject object, String key, boolean defaultValue){
        ...
        for(int i=0; i<itemArray.length(); i++) {
            try{
                ...
                if(obj != null){
                    ...
                    item.setTitle(name);
                    item.setPosition(i);
                    item.setPayed(checkChapterPayed(id));
                    list.add(item);
                }
            }catch(Exception e){
                e.printStackTrace();
...
//			boolean isBest = getBooleanValue(comic, JSON_KEY_BEST, false);
			int bestFlag = getIntValue(comic, JSON_KEY_FLAG, 1);
			item.setBest(bestFlag);
			int processType = getIntValue(comic, JSON_KEY_PROCESSTYPE, 2);
			item.setProcessType(processType);
